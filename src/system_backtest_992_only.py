...
LOOKBACK_DAYS = 180
ETF_CODE = "00992A"
OUTPUT_DIR = pathlib.Path("output_992")
ETF_CONTENTS_URL = "https://api.github.com/repos/4ru1013/capital-etf-00992a-portfolio/contents/data/out"
...
def list_holdings_files() -> list[dict]:
    ...
    files = [x for x in out if x["date"] >= START_DATE]
    if not files:
        raise RuntimeError(f"No {ETF_CODE} holdings files found from {START_DATE}")
    return files
...
def build_signals(holdings: dict[str, pd.DataFrame], prices: pd.DataFrame) -> pd.DataFrame:
    ...
    stocks = prices[~prices.code.isin(["TAIEX", ETF_CODE])]
    rows = []
...
def summarize(trades: pd.DataFrame, curve: pd.DataFrame):
    rows = []
    for col, label in [("strategy_value", "strategy"), ("taiex_value", "taiex"), ("etf992_value", "etf992")]:
...
def main():
    ...
    prices = pd.concat([fetch_price(c, start, end) for c in sorted(set(codes + ["TAIEX", ETF_CODE]))], ignore_index=True)
    ...
    curve["etf992_value"] = bh_curve(prices, ETF_CODE, dates, "etf992_value").values
    ...
    with pd.ExcelWriter(OUTPUT_DIR / "system_backtest_992.xlsx", engine="openpyxl") as w:
    ...
    print("[OK] 00992A-only backtest complete")
