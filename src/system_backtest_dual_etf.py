...
LOOKBACK_DAYS = 180
OUTPUT_DIR = pathlib.Path("output_dual_etf")
YAHOO_URL = "https://query1.finance.yahoo.com/v8/finance/chart/{symbol}"

ETF_SOURCES = {
    "00981A": "https://api.github.com/repos/4ru1013/united-etf-00981a-portfolio/contents/data/out",
    "00992A": "https://api.github.com/repos/4ru1013/capital-etf-00992a-portfolio/contents/data/out",
}
...
def list_holdings_files(etf_code: str) -> list[dict]:
    r = requests.get(ETF_SOURCES[etf_code], headers={"User-Agent": "Mozilla/5.0"}, timeout=60)
    r.raise_for_status()
    pat = re.compile(rf"^{etf_code}_holdings_(\d{{8}})\.csv$")
    out = []
    for it in r.json():
        m = pat.match(it.get("name", ""))
        if m:
            d = m.group(1)
            out.append({"date": f"{d[:4]}-{d[4:6]}-{d[6:]}", "key": d, "url": it["download_url"], "etf": etf_code})
    out.sort(key=lambda x: x["key"])
    return [x for x in out if x["date"] >= START_DATE]
...
def load_holdings(file_info: dict) -> pd.DataFrame:
    ...
    df = df.rename(columns={"weight": f"weight_{etf}", "shares": f"shares_{etf}", "name": f"name_{etf}"})
    return df[["signal_date", "code", f"name_{etf}", f"shares_{etf}", f"weight_{etf}", f"rank_{etf}", f"is_top10_{etf}"]]


def load_all_etf_holdings() -> dict[str, pd.DataFrame]:
    by_etf = {}
    for etf in ETF_SOURCES:
        files = list_holdings_files(etf)
        if not files:
            raise RuntimeError(f"No holdings files found for {etf} from {START_DATE}")
        by_etf[etf] = {f["date"]: load_holdings(f) for f in files}

    common_dates = sorted(set(by_etf["00981A"]).intersection(set(by_etf["00992A"])))
    if not common_dates:
        raise RuntimeError("No overlapping dates between 00981A and 00992A holdings")

    holdings = {}
    for d in common_dates:
        h981 = by_etf["00981A"][d]
        h992 = by_etf["00992A"][d]
        m = h981.merge(h992, on=["signal_date", "code"], how="outer")
        m["name"] = m.get("name_00981A").combine_first(m.get("name_00992A"))
        for etf in ETF_SOURCES:
            m[f"weight_{etf}"] = pd.to_numeric(m.get(f"weight_{etf}"), errors="coerce").fillna(0)
        ...
        m["combined_rank_score"] = 0
        for etf in ETF_SOURCES:
            rank = m[f"rank_{etf}"]
            m["combined_rank_score"] += np.where(rank.notna(), np.maximum(0, 101 - rank), 0)
        m["dual_etf_component"] = (
            50 * m["dual_holding"].astype(int)
            + 20 * m["top10_count"]
            + 0.3 * m["combined_rank_score"]
        ).clip(0, 100)
        m["etf_rank"] = m["dual_etf_component"].rank(method="first", ascending=False).astype(int)
        m["is_top10"] = m["etf_rank"] <= 10
        holdings[d] = m[[
...
def build_signals(holdings: dict[str, pd.DataFrame], prices: pd.DataFrame) -> pd.DataFrame:
    ...
    stocks = prices[~prices.code.isin(["TAIEX"] + list(ETF_SOURCES.keys()))]
    rows = []
    for d, h in holdings.items():
        ...
        m["setup_component"] = m["setup"].map({"A": 100, "B": 90, "C": 55, "D": 0}).fillna(0)
        m["trading_score"] = (
            0.55 * m["setup_component"]
            + 0.30 * m["market_component"]
            + 0.15 * m["dual_etf_component"].fillna(0)
        ).round(1)
        rows.append(m)
    if not rows:
...
def summarize(trades: pd.DataFrame, curve: pd.DataFrame):
    rows = []
    for col, label in [("strategy_value", "strategy"), ("taiex_value", "taiex"), ("etf981_value", "etf981"), ("etf992_value", "etf992")]:
...
def dual_stats(signals: pd.DataFrame, trades: pd.DataFrame):
    if trades.empty:
        return pd.DataFrame()
    cols = ["signal_date", "code", "dual_holding", "top10_count", "etf_count", "dual_etf_component"]
    x = trades.merge(signals[cols].drop_duplicates(["signal_date", "code"]), on=["signal_date", "code"], how="left")
    return x.groupby(["dual_holding", "top10_count"]).agg(trades=("code", "count"), win_rate=("return_pct", lambda s: (s > 0).mean()), avg_return=("return_pct", "mean"), median_return=("return_pct", "median"), total_pnl=("pnl", "sum")).reset_index()


def main():
    OUTPUT_DIR.mkdir(exist_ok=True)
    holdings = load_all_etf_holdings()
    all_holdings = pd.concat(holdings.values(), ignore_index=True)
    codes = sorted(set(all_holdings.code.astype(str)))
    start = (datetime.strptime(START_DATE, "%Y-%m-%d") - timedelta(days=LOOKBACK_DAYS)).date().isoformat()
    end = date.today().isoformat()
    prices = pd.concat([fetch_price(c, start, end) for c in sorted(set(codes + ["TAIEX"] + list(ETF_SOURCES.keys())))], ignore_index=True)
    prices.to_csv(OUTPUT_DIR / "price_panel.csv", index=False, encoding="utf-8-sig")
    all_holdings.to_csv(OUTPUT_DIR / "dual_holdings_panel.csv", index=False, encoding="utf-8-sig")
    ...
    curve["etf992_value"] = bh_curve(prices, "00992A", dates, "etf992_value").values
    ...
    dual = dual_stats(signals, trades)
    ...
    dual.to_csv(OUTPUT_DIR / "dual_etf_analysis.csv", index=False, encoding="utf-8-sig")
    with pd.ExcelWriter(OUTPUT_DIR / "system_backtest_dual_etf.xlsx", engine="openpyxl") as w:
        summary.to_excel(w, sheet_name="Summary", index=False)
        setup.to_excel(w, sheet_name="Setup_Analysis", index=False)
        dual.to_excel(w, sheet_name="Dual_ETF_Analysis", index=False)
        ...
        all_holdings.to_excel(w, sheet_name="Dual_Holdings", index=False)
    print("[OK] Dual ETF backtest complete")
